...
import { BaseTransport, TransportOptions, TransportState, MCPMessage } from './ITransport.js';
import { SSEParser, SSEEvent } from './SSEParser.js';
import { ZitiConnection, ZitiStream } from '../ziti/ZitiConnection.js';
...
export class ZitiSSETransport extends BaseTransport {
  ...
  private stream: ZitiStream | null = null;
  ...
  constructor(
    ...
    options: ZitiSSETransportOptions
  ) {
    super(options);
    ...
    this.messageQueue = new Queue<MCPMessage>();
  ...
  private setupParserListeners(): void {
    this.sseParser.on('event', (event: SSEEvent) => {
      this.handleSSEEvent(event);
    });

    this.sseParser.on('error', (error: Error) => {
   ...
   */
  private handleEndpointEvent(event: SSEEvent): void {
   ...
   */
  private handleMessageEvent(event: SSEEvent): void {
    try {
...
if (this.options.autoReconnect) {
   ...
   */
  async request(message: MCPMessage): Promise<unknown> {
    if (message.id === undefined) {
