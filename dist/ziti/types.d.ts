...
export interface ZitiSDK {
    ...
    httpRequest(serviceName: string, schemeHostPort: string | undefined, method: string, path: string, headers: string[], body: Buffer | undefined, onRequest: (() => void) | undefined, onResponse: ((obj: {
        status: number;
        headers: Record<string, string>;
    }) => void) | undefined, onResponseData: ((obj: {
        body: Buffer;
    }) => void) | undefined): void;
    dial(serviceName: string, isWebSocket: boolean, onConnect: (conn: number) => void, onData: (data: Buffer) => void): void;
    write(conn: number, data: Buffer, onWrite?: () => void): void;
    close(conn: number): void;
    serviceAvailable(serviceName: string, onServiceAvailable: (status: number) => void): void;
    setLogLevel(level: number): void;
}
export declare enum ZitiLogLevel {
    NONE = 0,
    ERROR = 1,
    WARN = 2,
    INFO = 3,
